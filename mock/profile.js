...
export default {
  ...
  'GET /api/profile/basic': (req, res) => {
    ...
    const projectOne = {
      ...
      technology: 'Spring Boot+Dubbo+Redis+MyBatis+HBase+Tidb+Kafka',
      projectContent: '1.营销大多有三部分内容，一是用户群体，二是资源，三是营销。本运营平台针对 三个方面，主要涉及的内容是，' +
        ...
        '搭建模块主要对 卡卷，理财金，会员金币，现金红包，􏰁额卷等福利的创建，以及短信，app push，语音外呼， 微信消息推送等营销手段。' +
      ...
      duties : '1.主要负责运营活动的资源搭建模块，活动搭建模块，数据发放模块，数据统计模\n' +
        ...
        '数据发放模块对于批量任务主要涉 及任务调度，以及多线程，异步请求的机制，并且需要保证数据库的一致性。实时发放数据主要通 过接收 kafka 消息，并且采用 tidb 的形式对数据进行存储，' +
        '并且采用 Redis 缓存机制来减少数据 库的压力。其中平台建设使用 redalert 监控机制对系统，对接口进行监控。',
    };
    const projectTwo = {
      time: '2016/10 - 2017/3',
